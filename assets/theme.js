...
// js/common/facets/sort-by-popover.js
var SortByPopover = class extends Popover {
  get shouldAppendToBody() {
    return false;
  }
};
...
/**
 * Generic, one-off PhotoSwipe lightbox opener, sharing the exact same look (background, close/prev/next
 * button styling, color-scheme) as <product-gallery>'s own zoom, but usable by anything that isn't a
 * <product-gallery> - eg. the media-blocks carousel block's own "expand" button. Unlike ProductGallery's
 * lightBox getter, this creates a fresh PhotoSwipeLightbox per call instead of caching one on an element,
 * since callers here don't reopen the same gallery repeatedly the way a product page does.
 *
 * Video items are handled through PhotoSwipe's contentLoad/contentDestroy hooks: rather than loading a
 * poster image, the actual <video-media> element already playing in the page is moved into the lightbox
 * slide (so playback position and mute state carry over with no restart), then moved back to
 * data.videoHome when the slide is torn down.
 *
 * @param {Array<object>} dataSource - PhotoSwipe items; video items need { type: 'video', videoElement,
 *   videoHome, width, height, thumbnailElement }, image items the usual { src, srcset, msrc, width, height,
 *   thumbnailElement }.
 * @param {number} index - index within dataSource to open on.
 * @param {string} colorSchemeId - color scheme id to paint the lightbox chrome with (matches
 *   zoom-color-scheme-id on <product-gallery>).
 */
function openMediaLightbox(dataSource, index, colorSchemeId) {
  const lightbox = new PhotoSwipeLightbox({
    pswpModule: () => import("photoswipe"),
    bgOpacity: 1,
    mainClass: `color-scheme--${colorSchemeId}`,
    closeTitle: window.themeVariables.strings.closeGallery,
    zoomTitle: window.themeVariables.strings.zoomGallery,
    errorMsg: window.themeVariables.strings.errorGallery,
    arrowPrev: false,
  ...
  lightbox.on("contentLoad", (event) => {
    ...
    content.element.className = "pswp-video-content";
    content.element.appendChild(content.data.videoElement);
    content.state = "loaded";
  });
  lightbox.on("contentDestroy", (event) => {
    const { content } = event;
    if (content.data.type === "video" && content.data.videoHome) {
      content.data.videoHome.prepend(content.data.videoElement);
    }
  });
  lightbox.init();
